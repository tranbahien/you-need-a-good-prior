...
from ..reparam_layers.gaussian_reparam_conv import GaussianConv2dReparameterization
...
def conv3x3(in_planes, out_planes, stride=1,
    ...
    return GaussianConv2dReparameterization(
        in_planes, out_planes, kernel_size=3, stride=stride,
        padding=1, bias=False, W_std=W_std, b_std=b_std,
        prior_per=prior_per, scaled_variance=scaled_variance)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 norm_layer='batchnorm', W_std=None, b_std=None,
...
class GaussianPreResNetReparameterization(nn.Module):
    def __init__(self, depth, num_classes=10, block_name='BasicBlock',
                 norm_layer='batchnorm', W_std=None, b_std=None,
                 prior_per="layer", scaled_variance=True):
        super(GaussianPreResNetReparameterization, self).__init__()
        ...
        self.fc = GaussianLinearReparameterization(
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                GaussianConv2dReparameterization(
