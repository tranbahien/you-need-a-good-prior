...
from ..reparam_layers.nflows_reparam_linear import NormFlowsLinearReparameterization
...
class MLPNormFlowsReparameterization(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, activation_fn,
                 n_transformations=4, scaled_variance=True, norm_layer=None):
        """Initialization.
        ...
        """
        super(MLPNormFlowsReparameterization, self).__init__()
        ...
        # Initialize layers
        self.layers = nn.ModuleList([NormFlowsLinearReparameterization(
            input_dim, hidden_dims[0], n_transformations=n_transformations, 
        ...
        for i in range(1, len(hidden_dims)):
            self.layers.add_module(
                "linear_{}".format(i), NormFlowsLinearReparameterization(
                    hidden_dims[i-1], hidden_dims[i], n_transformations=n_transformations, 
                    scaled_variance=scaled_variance))
            self.norm_layers.add_module(
                "norm_{}".format(i), init_norm_layer(hidden_dims[i],
                                                     self.norm_layer))

        self.output_layer = NormFlowsLinearReparameterization(
            hidden_dims[-1], output_dim, n_transformations=n_transformations, 
            scaled_variance=scaled_variance, bias=True)
