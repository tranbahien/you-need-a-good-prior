...
# These should be converted to use torch.densities
class Gaussian(Prior):
    ...
    def sample(self, shape=None):
        if shape is None:
            shape = self.mu.shape
        return self.mu + (self.var**0.5) * torch.randn(
            shape, dtype=self.mu.dtype, device=self.mu.device)

    def __str__(self):
        return "N("+str(self.mu.data.cpu().numpy()) + "," + str(
            self.var.data.cpu().numpy()) + ")"
...
class LogNormal(Prior):
    ...
    def sample(self, shape=None):
        if shape is None:
            shape = self.mu.shape
        return (self.mu + (self.var**0.5) * torch.randn(
            self.mu.shape, dtype=self.mu.dtype, device=self.mu.device)).exp()

    def __str__(self):
        return "logN("+str(self.mu.data.cpu().numpy()) + "," + str(
            self.var.data.cpu().numpy()) + ")"
...
class Gamma(Prior):
    ...
    def sample(self, shape=None):
        ...
        return torch.as_tensor(np.random.gamma(
            self.shape, self.scale, size=shape), dtype=self.shape.dtype)

    def __str__(self):
        return "Ga("+str(self.shape.data.cpu().numpy()) + "," + str(
            self.scale.data.cpu().numpy()) + ")"
...
class Laplace(Prior):
    ...
    def sample(self, shape=None):
        if shape is None:
            shape = self.mu.shape
        return torch.as_tensor(np.random.laplace(
            self.mu, self.sigma, size=shape), device=self.mu.device,
                               dtype=self.mu.dtype)

    def __str__(self):
        return "Lap.("+str(self.mu.data.cpu().numpy()) + "," + str(
...
class Beta(Prior):
    ...
    def logp(self, x):
        return torch.sum(densities.beta(self.a, self.b, x))

    def sample(self, shape=None):
        if shape is None:
            shape = self.beta.shape
        return torch.as_tensor(np.random.beta(self.a, self.b, size=shape),
                               device=self.a.device, dtype=self.a.dtype)

    def __str__(self):
        return "Beta(" + str(self.a.data.cpu().numpy()) + "," + str(
            self.b.data.cpu().numpy()) + ")"
...
class Uniform(Prior):
    ...
    def sample(self, shape=None):
        if shape is None:
            shape = self.lower.shape
        return self.lower + (self.upper - self.lower) *\
             self.lower.new(shape).uniform_()

    def __str__(self):
        return "U("+str(self.lower.data.cpu().numpy()) + "," + str(
            self.upper.data.cpu().numpy()) + ")"
